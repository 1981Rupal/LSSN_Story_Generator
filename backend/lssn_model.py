...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, c_text, c_image, mode="fused"):
        ...
        for layer in self.layers:
            x = layer(x, c_text, c_image, mode=mode)
...
class LSSN_UNet(nn.Module):
    ...
    def forward(self, x, timesteps, c_text, c_image, return_features=False, mode="fused"):
        """
        x: (B, C, H, W) -- must be the NOISED latent x_t, not the clean latent x_0.
        timesteps: (B,)
        ...
        mode: "fused" | "text_only" | "image_only" -- forwarded to every SynchronizationModule.
              Use "text_only"/"image_only" (with the corresponding c_* still passed, the other
              ignored) when isolating a single modality for the invariance loss, so the gate
              does not rescale the two branches asymmetrically. See lssn_modules.py.
        """
